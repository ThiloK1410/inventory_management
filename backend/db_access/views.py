...
class DeliveryViewSet(viewsets.ViewSet):
    ...
    def retrieve(self, request, pk=None):
        ...
        serializer = DeliverySerializer(user)
        return Response(serializer.data)
...
def inventoryAccess(request, id=None):
    ...
    if id is None:
        if request.method == "GET":
            ...
            return Response(
                data="this request method is not allowed when accessing whole model",
    ...
    # access on one element of model
        ...
        if not (isinstance(id, int) and id >= 0):
            return Response(
                data="the id needs to be a positive integer",
                status=status.HTTP_400_BAD_REQUEST,
            )
        try:
            ...
            return Response(
                data="given id is not in Inventory", status=status.HTTP_404_NOT_FOUND
            )
    ...
    if request.method == "GET":
        ...
        return Response(
            data="this request method is not allowed when accessing a single model item",
            status=status.HTTP_400_BAD_REQUEST,
        )
